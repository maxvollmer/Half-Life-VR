...
void TeamFortressViewport::Initialize(void)
{
	...
	strcpy_s(m_sMapName, "");
	strcpy_s(m_szServerName, "");
	for (int i = 0; i < 5; i++)
	{
		m_iValidClasses[i] = 0;
		strcpy_s(m_sTeamNames[i], "");
	}
	...
	try
	{
		...
		while ((strlen(token) > 0) && (m_iNumMenus < MAX_MENUS))
		{
			...
			while (token[0] != '}' && (strlen(token) > 0))
			{
				...
				strcpy_s(szLastButtonText, cText);
...
void TeamFortressViewport::UpdateSpectatorPanel()
{
	...
	if (g_iUser1 && gHUD.m_pCvarDraw->value && !gHUD.m_iIntermission)  // don't draw in dev_overview mode
	{
		...
		if (!m_pSpectatorPanel->isVisible())
		{
			...
			_snprintf_s(tempString, sizeof(tempString) - 1, "%c%s", HUD_PRINTCENTER, CHudTextMessage::BufferedLocaliseTextString("#Spec_Duck"));
		...
		if (gHUD.m_Spectator.m_autoDirector->value)
		{
			...
			strcpy_s(helpString2, tempString);
		}
		...
		if (gEngfuncs.IsSpectateOnly())
		{
			...
			_snprintf_s(szText, 63, "%s: %d", CHudTextMessage::BufferedLocaliseTextString("#Spectators"), gHUD.m_Spectator.m_iSpectatorNumber);
		}
		else
		{
			// otherwise show map name
			char szMapName[64];
			COM_FileBase(gEngfuncs.pfnGetLevelName(), szMapName, 64);

			_snprintf_s(szText, 63, "%s: %s", CHudTextMessage::BufferedLocaliseTextString("#Spec_Map"), szMapName);
		}
...
CMenuPanel* TeamFortressViewport::CreateTextWindow(int iTextToShow)
{
	...
	CMenuPanel* pMOTDPanel = nullptr;

	// if we're in the game (ie. have selected a class), flag the menu to be only grayed in the dialog box, instead of full screen
	if (cText)
	{
		pMOTDPanel = CMessageWindowPanel_Create(cText, cTitle, g_iPlayerClass == PC_UNDEFINED, false, 0, 0, ScreenWidth, ScreenHeight);
...
int TeamFortressViewport::MsgFunc_TeamScore(const char* pszName, int iSize, void* pbuf)
{
	...
	int i = 1;
	for (; i <= m_pScoreBoard->m_iNumTeams; i++)
	{
