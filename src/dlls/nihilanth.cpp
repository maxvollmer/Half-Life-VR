...
public:
	...
	char m_szRechargerTarget[64] = { 0 };
	char m_szDrawUse[64] = { 0 };
	char m_szTeleportUse[64] = { 0 };
	char m_szTeleportTouch[64] = { 0 };
	char m_szDeadUse[64] = { 0 };
	char m_szDeadTouch[64] = { 0 };
...
void CNihilanth::Spawn(void)
{
	...
	if (m_szRechargerTarget[0] == '\0')
		strcpy_s(m_szRechargerTarget, "n_recharger");
	if (m_szDrawUse[0] == '\0')
		strcpy_s(m_szDrawUse, "n_draw");
	if (m_szTeleportUse[0] == '\0')
		strcpy_s(m_szTeleportUse, "n_leaving");
	if (m_szTeleportTouch[0] == '\0')
		strcpy_s(m_szTeleportTouch, "n_teleport");
	if (m_szDeadUse[0] == '\0')
		strcpy_s(m_szDeadUse, "n_dead");
	if (m_szDeadTouch[0] == '\0')
		strcpy_s(m_szDeadTouch, "n_ending");
	...
	if (m_flShootEnd > gpGlobals->time)
	{
		...
		while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->time)
		{
			if (m_hEnemy != nullptr)
			{
				...
				pEntity = CBaseEntity::Create<CNihilanthHVR>("nihilanth_energy_ball", vecSrc, pev->angles, edict());
