...
char* GetTGANameForRes(const char* pszName)
{
	...
	sprintf_s(gd, "gfx/vgui/%s.tga", sz);
	return gd;
}
...
void CommandButton::RecalculateText(void)
{
	...
	if (m_cBoundKey != 0)
	{
		...
		{
			strcpy_s(szBuf, m_sMainText);
		}
		else
		{
			sprintf_s(szBuf, "  %c  %s", m_cBoundKey, m_sMainText);
		}
...
{
	strncpy_s(m_sMainText, text, MAX_BUTTON_SIZE);
...
void CImageLabel::LoadImage(const char* pImageName)
{
	...
	if (m_pTGA == nullptr)
	{
		...
		char sz[256];
		sprintf_s(sz, "%s/%s", gEngfuncs.pfnGetGameDirectory(), pImageName);
