...
static byte* LoadFileByResolution(const char* filePrefix, int xRes, const char* filePostfix)
{
	...
	while (1)
	{
		// try load
		char fname[256];
		sprintf_s(fname, "%s%d%s", filePrefix, g_ResArray[resNum], filePostfix);
...
{
	int r, g, b, a;
	sscanf_s(colorVector, "%d %d %d %d", &r, &g, &b, &a);
	colorArray[0] = r;
	colorArray[1] = g;
	colorArray[2] = b;
	colorArray[3] = a;
...
CSchemeManager::CSchemeManager(int xRes, int yRes)
{
	...
	while (strlen(token) > 0 && (currentScheme < numTmpSchemes))
	{
		...
		strncpy_s(paramName, token, tokenSize);
		...
		// get paramValue
		pFile = gEngfuncs.COM_ParseFile(pFile, token);
		strncpy_s(paramValue, token, tokenSize);
		...
		if (!_stricmp(paramName, "SchemeName"))
		{
			...
			if (pScheme)
			{
				...
				{
					strcpy_s(pScheme->fontName, "Arial");
				}
			}
			...
			strncpy_s(pScheme->schemeName, paramValue, CScheme::SCHEME_NAME_LENGTH);
		...
		{
			strncpy_s(pScheme->fontName, paramValue, CScheme::FONT_NAME_LENGTH);
...
buildDefaultFont:
	...
	{
		currentScheme = 0;
		strcpy_s(tmpSchemes[0].schemeName, "Default Scheme");
		strcpy_s(tmpSchemes[0].fontName, "Arial");
	...
	for (int i = 0; i < m_iNumSchemes; i++)
	{
		...
		for (int j = 0; j < i; j++)
		{
			...
			if (!_stricmp(m_pSchemeList[i].fontName, m_pSchemeList[j].fontName) && m_pSchemeList[i].fontSize == m_pSchemeList[j].fontSize && m_pSchemeList[i].fontWeight == m_pSchemeList[j].fontWeight)
			{
		...
		if (!m_pSchemeList[i].font)
		{
			...
			{
				sprintf_s(fontFilename, "gfx\\vgui\\fonts\\%d_%s.tga", m_xRes, m_pSchemeList[i].schemeName);
