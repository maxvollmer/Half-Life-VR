...
static CBasePlayer* FindPlayerByName(const char* pTestName)
{
	...
	{
		CBasePlayer* pPlayer = CBaseEntity::SafeInstance<CBasePlayer>(g_engfuncs.pfnPEntityOfEntIndex(i));
		if (pPlayer)
		{
			const char* pNetName = STRING(pPlayer->pev->netname);
...
static void VoiceServerDebug(char const* pFmt, ...)
{
	...
	_vsnprintf_s(msg, sizeof(msg), pFmt, marker);
	va_end(marker);
