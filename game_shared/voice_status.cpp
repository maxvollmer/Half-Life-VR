	...
	Panel** pParentPanel)
{
	...
	m_BlinkTimer = 0;
	m_VoiceHeadModel = HSPRITE_VALVE(0);
	...
	strcpy_s(m_pchGameDir, strlen(pchGameDirT) + 1, pchGameDirT);
...
void CVoiceStatus::UpdateSpeakerStatus(int entindex, qboolean bTalking)
{
	...
	{
		char msg[256];
		_snprintf_s(msg, sizeof(msg), "CVoiceStatus::UpdateSpeakerStatus: ent %d talking = %d\n", entindex, bTalking);
		...
		if (bTalking)
		{
			...
			if (!pLabel)
			{
				if (pLabel = GetFreeVoiceLabel())
				{
					...
					char paddedName[512];
					_snprintf_s(paddedName, sizeof(paddedName), "%s   ", info.name);
...
void CVoiceStatus::UpdateServerState(bool bForce)
{
	...
	if (bForce || m_bServerModEnable != bCVarModEnable)
	{
		...
		char str[256];
		_snprintf_s(str, sizeof(str), "VModEnable %d", m_bServerModEnable);
		ServerCmd(str);
